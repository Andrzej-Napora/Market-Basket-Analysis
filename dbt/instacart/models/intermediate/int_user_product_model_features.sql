SELECT
    ...
    coalesce(lo.last_9_orders,0) as last_9_orders,

    coalesce(lo.last_12_orders,0) as last_12_orders,
...
left JOIN {{ref('int_last_orders')}} AS lo
    ON ups.user_id = lo.user_id
    AND ups.product_id = lo.product_id
